...
LABEL org.opencontainers.image.title="impbot-pwa" \
      org.opencontainers.image.description="PWA de controle do robô aplicador de impermeabilizante IMP-BOT" \
...
RUN mkdir -p /opt/impbot && cat > /opt/impbot/serve.py <<'PYEOF'
#!/usr/bin/env python3
...
if __name__ == "__main__":
    ...
    with socketserver.TCPServer(("0.0.0.0", port), Handler) as httpd:
        httpd.serve_forever()
PYEOF
RUN chmod +x /opt/impbot/serve.py
